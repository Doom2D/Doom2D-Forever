...
begin
 ...
 gPlayer1 := g_Player_Get(g_Player_Create('Doomer', _RGB(64, 208, 64), TEAM_NONE, False, PLAYERNUM_1));
 ...
 if TwoPlayers then
 begin
  gPlayer2 := g_Player_Get(g_Player_Create('Doomer', _RGB(96, 96, 96), TEAM_NONE, False, PLAYERNUM_2));
  if gPlayer2 = nil then
  begin
...
begin
 ...
 if gGameSettings.GameType = GT_CUSTOM then
 begin
  if LowerCase(P[0]) = 'map' then if (Length(P) > 1) and gGameOn then
  begin
...
begin
 ...
 begin
  ...
  g_Game_Pause(True);
 end else if (g_ActiveWindow <> nil) and not Show then g_Game_Pause(False);
end;
