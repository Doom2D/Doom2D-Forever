...
const
  KEYTABLE: array[0..255] of string = (
    ...
    'D', 'F', 'G', 'H', 'J', 'K', 'L', ';',  // 32
    '', '', 'LShift', '\', 'Z', 'X', 'C', 'V',  // 40
    'B', 'N', 'M', ',', '.', '/', 'RShift', 'Серый *',  // 48
    'LAlt', 'Space', 'CapsLock', 'F1', 'F2', 'F3', 'F4', 'F5',  // 56
    'F6', 'F7', 'F8', 'F9', 'F10', '', 'Scroll Lock', 'Серый 7',  // 64
    'Серый 8', 'Серый 9', 'Серый -', 'Серый 4', 'Серый 5', 'Серый 6', 'Серый +', 'Серый 1',  // 72
    'Серый 2', 'Серый 3', 'Серый 0', 'Серый .', '', '', '', 'F11',  // 80
    'F12', '', '', '', '', '', '', '',  // 88
    '', '', '', '', '', '', '', '',  // 96
    '', '', '', '', '', '', '', 'Серый /',  // 104
    '', '', '', '', '', '', '', '',  // 112
    ...
    '', '', '', '', '', '', '', '',  // 136
    '', '', '', '', '', '', '', '',  // 144
    '', '', '', '', 'Серый Enter', 'RCtrl', '', '',  // 152
    '', '', '', '', '', '', '', '',  // 160
    '', '', '', '', '', '', '', '',  // 168
    '', '', '', '', '', '', '', 'Print Screen',  // 176
    'RAlt', '', '', '', '', '', '', '',  // 184
    '', '', '', '', '', '', '', 'Home',  // 192
    'Вверх', 'Page Up', '', 'Влево', '', 'Вправо', '', 'End',  // 200
    'Вниз', 'Page Down', 'Insert', 'Delete', '', '', '', '',  // 208
    '', '', '', '', '', '', '', '',  // 216
    '', '', '', '', '', '', '', '',  // 224
    '', '', '', '', '', '', '', '',  // 232
    '', '', '', '', '', '', '', '',  // 240
...
begin
 ...
 if Up then g_Texture_Get(BSCROLL_UPA, ID)
  else g_Texture_Get(BSCROLL_UPU, ID);
 e_Draw(ID, X, Y, 0, False, False);

 if Down then g_Texture_Get(BSCROLL_DOWNA, ID)
  else g_Texture_Get(BSCROLL_DOWNU, ID);
 ...
 if FBackTexture <> '' then
  if g_Texture_Get(FBackTexture, ID) then
   e_DrawSize(ID, 0, 0, 0, False, False, gScreenWidth, gScreenHeight)
    else e_Clear(GL_COLOR_BUFFER_BIT, 0.5, 0.5, 0.5);
...
var
  w, h: Word;
begin
 if FFixedLen = 0 then FFont.GetTextSize(FText, w, h)
  else w := e_CharFont_GetMaxWidth(FFont.ID)*FFixedLen; 
 Result := w; 
end;
  ...
  begin
   if (ControlType = TGUIListBox) or (ControlType = TGUIFileListBox) then
    h := h+(FItems[a].Control as TGUIListBox).GetHeight()
     else h := h+e_CharFont_GetMaxHeight(FFontID);
  end;
     ...
     VK_RETURN:
      begin
       if not FIsQuery then
        with FWindow do if FActiveControl <> Self then SetActive(Self);
  ...
  begin
   ...
   if a = FIndex then
    FFont.Draw(FX+4, FY+4+(a-FStartLine)*16, s, FActiveColor.R, FActiveColor.G, FActiveColor.B)
     else FFont.Draw(FX+4, FY+4+(a-FStartLine)*16, s, FUnActiveColor.R, FUnActiveColor.G, FUnActiveColor.B);
  end;
end;
